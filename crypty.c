...
// 
// Type ./crypty to obtain command line help.
//
// Pure C99 code,
...
typedef enum {	
				CRYPTY_V3=3
				} crypty_Versions_Constants;

crypty_Versions_Constants cryptyVersion;

const char CRYPTY_V3_EXTENSION[]= ".$#3";
...
int local_crypty(int argc, char* argv[]);
...
jint Java_com_example_crypty_MainActivity_crypty( JNIEnv*  env, jobject  thiz,  jcharArray jargv )
{    //jargv is a Java array of Java strings
	int argc = (*env)->GetArrayLength( env, jargv );
    //__android_log_print(ANDROID_LOG_DEBUG, "CRYPTY", "%d", argc);
    //typedef char* pchar;
    ...
    for(i=0; i<argc; i++)
    {
        ...
        //__android_log_print(ANDROID_LOG_DEBUG, "CRYPTY", "%s", argv[i]);
    }

    //Call main
    result = local_crypty(argc, argv);
    //__android_log_print(ANDROID_LOG_DEBUG, "CRYPTY", "%d", result);
...
#endif
{
	...
	*/
	...
	if (argc>=5) {
		...
		// decrypting: so crypty Version can be deduced from extension:
		if (strlen(szFile)>=4)
					if (strcmp( szFile+(strlen(szFile)-4), CRYPTY_V3_EXTENSION ) == 0 )
						cryptyVersion=CRYPTY_V3;
					else {
	...
	if (bEncrypt && !bOutputToTheSameFile) {
		...
		if (cryptyVersion==CRYPTY_V3)
			memcpy(szNewFile+strlen(szNewFile), CRYPTY_V3_EXTENSION, 4);
	}
	...
	do
	{
		...
		if (nBytesRead || feof(hFile) )
		{
			...
			// to remove the crypty tail, or :
			// CRYPTY_V3: Output File Size will be greater than original data file.
			if (!bEncrypt && 
